...
def qoi_vs_bit_rate(plot_dir, rows, y_col, ylabel, filename, *, log_y=True):
    by_comp_points = {}
    for r in rows:
...
def one_minus_qoi_vs_bit_rate(plot_dir, rows, y_col, ylabel, filename):
    by_comp_points = {}
    for r in rows:
        comp = r.get("compressor name", "").strip() or "default"
        x = to_float(r.get("bit_rate"))
    ...
    ax.set_yscale("log")
    ax.set_xlabel("Bit rate", fontsize=12, color="black")
    ax.set_ylabel(f"1 - {ylabel}", fontsize=12, color="black")
    ax.tick_params(axis="both", labelsize=10, colors="black")
    for spine in ax.spines.values():
        spine.set_color("black")
    leg = ax.legend(fontsize=10)
...
def scatter_xy_by_compressor(plot_dir, rows, x_col, y_col, xlabel, ylabel, filename, *, log_y=True):
    by_comp_points = {}
    for r in rows:
        comp = r.get("compressor name", "").strip() or "default"
